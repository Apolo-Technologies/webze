...
var IpcProvider = SandboxedModule.require('../lib/webze/ipcprovider', {
    requires: {
...
describe('lib/webze/ipcprovider', function () {
    describe('send', function () {
        it('should send basic request', function () {
            var provider = new IpcProvider('', net);
            var result = provider.send({ id: 1, method: 'zrm_test'});
    ...
    describe('sendAsync', function () {
        it('should send basic async request', function (done) {
            var provider = new IpcProvider('', net);
            provider.sendAsync({ id: 1, method: 'zrm_test'}, function (err, result) {
                assert.isObject(result);
                done();
            });
        }); 
